...
class BaseEstimator(object):
    ...
    def set_sample_rate(self, sample_rate):
        assert (
            sample_rate in AUDIO_SAMPLE_RATES
        ), f"Check audio sample rates in {AUDIO_SAMPLE_RATES}."
        self.config["sample_rate"] = sample_rate

    def set_scales(self, scales):
        assert all(
            scale > 0 and scale < 1 for scale in scales
        ), "Observable scales should be a list of positive values in (0,1]."
