...
class INS(BaseEstimator):
    ...
    def set_num_surrogates(self, num_surrogates):
        assert (
            isinstance(num_surrogates, int) and num_surrogates > 0
        ), "Number surrogates must be positive integer."
    ...
    def __call__(self, audio_path):
        ...
        for Nh0 in tqdm(self.config["scales"]):
            ...
            MSp = tf_rep.tfrsp_hm(
                x_hilbert, tt, Nfft, Nh, self.config["Mh"], self.config["tm"]
            )
            ...
            for jj in np.arange(self.config["num_surrogates"]):
                ...
                MSp = tf_rep.tfrsp_hm(
                    z_hilbert, tt, Nfft, Nh, self.config["Mh"], self.config["tm"]
                )
            ...
            gamma_thresh = scipy.stats.gamma.ppf(
                1 - self.config["fa_rate"], gamma_a, gamma_loc, gamma_scale
            )
